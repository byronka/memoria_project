...
public class DiffMatchPatch {
     ...
     * @return Linked List of Diff objects.
     */
    private List<Diff> diff_main(String text1, String text2,
     ...
     */
    List<Diff> diff_bisect(String text1, String text2,
                                           long deadline) {
     ...
     * Convert a Diff list into a pretty HTML report.
     * @param diffs List of Diff objects
     * @param maxLength the maximum length of an EQUALS section before we
     *                  truncate it with ellipsis
     * @return HTML representation.
     */
    public String diff_prettyHtml(List<Diff> diffs, int maxLength) {
        StringBuilder html = new StringBuilder();
        for (Diff aDiff : diffs) {
            String text = aDiff.text.replace("&", "&amp;").replace("<", "&lt;")
                    .replace(">", "&gt;").replace("\n", "<br>");
            switch (aDiff.operation) {
                ...
                case EQUAL:
                    if (text.length() > maxLength * 5) {
                        text = text.substring(0, maxLength) + String.format(" ... (%,d characters) ... ", text.length() - (maxLength * 2)) + text.substring(text.length() - maxLength);
                    }
                    html.append("<span class=\"equal\">")
                            .append(text)
                            .append("</span>");
                    break;
            }
     ...
     */
    @Deprecated public List<Patch> patch_make(String text1, String text2,
                                                    List<Diff> diffs) {
        return patch_make(text1, diffs);
    }
    ...
    public List<Patch> patch_deepCopy(List<Patch> patches) {
        List<Patch> patchesCopy = new ArrayList<>();
    ...
    public List<Patch> patch_fromText(String textline)
        ...
        List<String> text = new ArrayList<>(textList);
        Patch patch;
        Pattern patchHeader
