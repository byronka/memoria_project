...
// find existing relations to add, used on a person's edit page.
class RelationSearch {

    // bind the list of results to a variable
    searchResultList;

    // a NodeList of elements on the page that are inputs for adding
    // relations - siblings, parents, spouses, children - to a person.
    linkageInputs;
    ...
    // the reset button for the current form.  This is enabled when there
    // is data in the input.
    currentResetButton;

    // A button for indicating we are submitting a new or existing person
    // as a relation
    currentSubmitButton;

    // The hidden input we'll set to another person if the user selects an
    // existing person.
    otherPersonIdInput;
    ...
    constructor() {
        this.linkageInputs = document.querySelectorAll('.linkages')
    ...
    addListener = () => {
        for (let i = 0; i < this.linkageInputs.length; i++) {
            ...
            this.linkageInputs[i].addEventListener("keyup", this.searchHandler);
            ...
            this.linkageInputs[i].addEventListener("keydown", this.arrowsHandler);
            ...
            this.linkageInputs[i].addEventListener("search", this.searchHandler);
     ...
     * in the proper place.
     * @param personDataElement is data that is sent back by the server
     *                          about a person, specifically their name and id,
     *                          which is used to fill in information when we
     *                          move to the next step.
     */
    setSelectedDataInsideInput = (personDataElement) => {
        this.currentInput.value = personDataElement.getAttribute('data-personname');
        this.otherPersonIdInput.value = personDataElement.getAttribute('data-personid')

        // make the input read-only - editing the name no longer makes sense,
        // since the form will be using the id and not the name, so editing the
        // name does nothing after this happens.  The reset button clears this.
        this.currentInput.setAttribute('readonly', 'readonly');
        this.currentSubmitButton.value = 'Connect existing person'
        this.currentResetButton.disabled = false;

        // create local variables for use in subsequent event handler, so
        // we aren't reliant on shifting global variables (aka class properties)
        let currentInput = this.currentInput;
        let otherPersonIdInput = this.otherPersonIdInput;
        let currentResetButton = this.currentResetButton;
        let currentSubmitButton = this.currentSubmitButton;

        this.currentResetButton.addEventListener('click', () => {
            // when a user clicks this reset button, clear all the inputs and
            // re-enable editing, and disable the reset button
            currentInput.value = '';
            otherPersonIdInput.value = '';
            currentInput.removeAttribute('readonly');
            currentResetButton.disabled = true;
            currentSubmitButton.value = 'Connect new person'
        });
    ...
    searchHandler = (e) => {
        ...
        if (this.isShowingSearchResults && (e.key == 'ArrowDown' || e.key == 'ArrowUp')) {
            return false;
        }
    ...
    replaceSearchResults = (currentSearch, targetElement) => {
      // if they aren't searching for anything, just return
        if (!currentSearch) {
            ...
            if (this.otherPersonIdInput) {
                this.otherPersonIdInput.value = null;
                this.otherPersonIdInput = null;
            }
            return;
        }
        ...
        // find the proper hidden input by going to the containing form and then back down
        const containingForm = this.currentInput.closest('form.add-relation-action');
        this.otherPersonIdInput = containingForm.querySelector('input[name="other_person_id"]');
        this.currentResetButton = containingForm.querySelector('.add-relation-reset-button');
        this.currentSubmitButton = containingForm.querySelector('.add-relation-submit-button')
        /*
        ...
        fetch(
            ...
            .then((response) => {
                ...
                if (this.resultsCount > 0) {
                    ...
                    // move the box rightwards if we're too close to the left edge
                    const rect = parentDiv.getBoundingClientRect();
                    if (rect.left < 100) {
                        this.searchResultContainer.style.right = '-265px';
                    } else if (rect.left < 280) {
                        this.searchResultContainer.style.right = '-180px';
                    } else {
                        this.searchResultContainer.style.right = '25px';
                    }
...
addEventListener("load", (event) => {
    const relationSearch = new RelationSearch();
    relationSearch.addListener();
});
